<template>
    <p>快捷回复</p>
    <el-input v-model="inputValue" @keyup.enter="handleAddTag" placeholder="输入标签" class="input">
        <template #append>
            <el-button class="icon-btn" :icon="Plus" @click="handleAddTag" />
        </template>
    </el-input>
    <el-tag v-for="tag in replyList" :key="tag" class="mx-1" closable :disable-transitions="false"
...
watch(() => store.replyList, () => { store.saveReplyList() }, { deep: true })
